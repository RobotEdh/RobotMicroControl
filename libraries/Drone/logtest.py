   ...
   while (not found and not eof ):
       ...
       if ((len(char0) == 0) or (len(char1) == 0)):
           ...
           char0 = fichier.read(4)  # 4 fillers
              ...
              print("throttle: ",int.from_bytes(char0, byteorder='little',signed=False))
              print("motor0: ",int.from_bytes(char1, byteorder='little',signed=False))
              char2 = fichier.read(1) # motor1
              char3 = fichier.read(1) # motor2
              char4 = fichier.read(1) # motor3
              fourbytes = fichier.read(4) # tick 32 bytes
              print("tick 32 bytes: ",int.from_bytes(fourbytes, byteorder='little',signed=False))
           ...
           if (foundPID):
              print("angle type: ",int.from_bytes(char0, byteorder='little',signed=False))
              print("angle: ",int.from_bytes(char1, byteorder='little',signed=False))
              char2 = fichier.read(1) # RC_commandRP
              #print("RC_commandRP: ",int.from_bytes(char2, byteorder='little',signed=False))
              char3 = fichier.read(1) # error
              #print("error: ",int.from_bytes(char3, byteorder='little',signed=False))
              char4 = fichier.read(1) # sum_error
              #print("sum_error: ",int.from_bytes(char4, byteorder='little',signed=False))
              char5 = fichier.read(1) # delta_error
              #print("delta_error: ",int.from_bytes(char5, byteorder='little',signed=False))
              char6 = fichier.read(1) # anglePID
              #print("anglePID: ",int.from_bytes(char6, byteorder='little',signed=False))
              char7 = fichier.read(1) # sampleTime
              print("sampleTime: ",int.from_bytes(char7, byteorder='little',signed=False))
              fourbytes = fichier.read(4) # tick 32 bits
              print("tick 32 bytes: ",int.from_bytes(fourbytes, byteorder='little',signed=False))
              ...
              if ((int.from_bytes(fourbytes, byteorder='little',signed=False) > 0) and (int.from_bytes(fourbytes, byteorder='little',signed=False) < 900000000)):    # tick>0
                ...
                piddumpsize = piddumpsize+2+6+4 # 12 bytes
