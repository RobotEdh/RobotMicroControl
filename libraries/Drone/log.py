   ...
   while (not found and not eof ):
       ...
       if ((len(char0) == 0) or (len(char1) == 0)):
           ...
           char0 = fichier.read(3)  # 3 fillers
              ...
              TlogMotor[iMotor,0] = int.from_bytes(fourbytes, byteorder='big',signed=False)
           ...
           if (foundPID):
              ...
              fourbytes = fichier.read(4) # tick 32 bytes
              TlogPID[iPID,0] = int.from_bytes(fourbytes, byteorder='big',signed=True)
              ...
              if (ord(char0) == 0):                  #Roll
                    ...
                    TlogPIDPitch[iPIDPitch,0] = int.from_bytes(fourbytes, byteorder='big',signed=True)
                    ...
                    TlogPIDYaw[iPIDYaw,0] = int.from_bytes(fourbytes, byteorder='big',signed=True)
                    ...
                    TlogPIDYaw[iPIDYaw,3] = int.from_bytes(char3, byteorder='big',signed=True)
                    TlogPIDYaw[iPIDYaw,4] = int.from_bytes(char4, byteorder='big',signed=True)
                    TlogPIDYaw[iPIDYaw,5] = int.from_bytes(char5, byteorder='big',signed=True)
                    TlogPIDYaw[iPIDYaw,6] = int.from_bytes(char6, byteorder='big',signed=True)
                    iPIDYaw = iPIDYaw +1                  
