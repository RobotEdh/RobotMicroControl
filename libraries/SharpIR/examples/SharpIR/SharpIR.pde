...
void setup() {
  ...
  SharpIR.SharpIR_init((long)1080); // initialize the default pin A0 connected to the Sharp IR GP2Y0A21Y
...
void loop() {
  ...
  double distance=SharpIR.SharpIR_distance();  // this returns the distance to the object you're measuring
  
  unsigned long time2=millis()-time1;  // the following gives you the time taken to get the measurement

  Serial.print("Distance (mm): ");  // returns it to the serial monitor
